...
class AssignmentsService
{
  ...
  public function addPersonToProject($project_id, $person_id, $conn){
    $sql = "INSERT INTO ProjectAssignments(project_id, person_id) VALUES (:project_id, :person_id)";
    $stmt = $conn->prepare($sql);
    $stmt->bindValue("project_id", $project_id);
  ...
  public function removePersonFromProject($project_id, $person_id, $conn){
    $sql = "DELETE FROM ProjectAssignments WHERE project_id = :project_id AND person_id = :person_id";
    $stmt = $conn->prepare($sql);
    $stmt->bindValue("project_id", $project_id);
    $stmt->bindValue("person_id", $person_id);
    $stmt->execute();
